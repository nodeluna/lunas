...
namespace about {
	...
	inline std::string help =
	    ...
	    "        the port number. if this option was used, it must come after the relevant remote path. It defaults to 22, if the "
	    "option is not used\n"
	    ...
	    "        example: -p /dir1/dir2/ -x file_inside_dir2    # a /full/path or regular ./relative/paths aren't allowed, and would "
	    "get ignored\n"
	    "\n"
	    ...
	    "        prevent a partition from filling up by setting a threshold to prevent syncing files to it once it's reached. default "
	    "is 1GiB\n"
	    ...
	    "        the destination file, if exists, gets removed then re-synced again. it replaces old mtime files with newer mtime "
	    "ones\n"
