...
namespace luco {
	...
	bool luco::duplicate_nest(const std::string& nest_name, const size_t& line_number) {
		if (auto it = ldata.find(nest_name); it != ldata.end()) {
			luco::strerror(
			    "duplicate nest '" + nest_name.substr(0, nest_name.size() - 2) + "' line: " + std::to_string(line_number), -1);
			return false;
		}
		return true;
	}
		...
		for (size_t i = 0; i < data.size(); i++) {
			...
			if (tokentype & token_type::COMMENT || skipping_nest) {
				if (tokentype & token_type::NEST_NAME) {
					...
					nest_stack.push(
					    {nest_val.find("::") != nest_val.npos ? nest_val.substr(0, nest_val.size() - 2) : nest_val,
						line_number});
				}
