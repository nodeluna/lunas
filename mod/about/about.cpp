...
export namespace lunas
{
	namespace about
	{
		...
		inline std::string help =
		    ...
		    "\n"
		    "    [-sr, --sftp-retries] <times>\n"
		    "        if the ssh server was disconnected while copying a file, retry x times to recopy it before moving "
		    "on to the next file\n"
		    "        -> check out --sftp-timeout\n"
		    "        -> default is 6 retries\n"
		    "\n"
		    "    [-st, --sftp-timeout] <seconds>\n"
		    "        if the ssh server was disconnected while copying a file, sleep x seconds between retries\n"
		    "        -> check out --sftp-retries\n"
		    ...
		    "\n"
		    "    [-nr, --no-recursive]\n"
		    "        do not recursively sync input directories regardless of the other options used\n"
		    "\n"
		    ...
		    "        allow a path to be synced. this option can be used many times to *allow* multiple "
		    "directories/files\n"
		    "        example: -r user@ip:/dir1/dir2/ -a dir3/dir    # only relative path starting from inside the input dir is "
		    "allowed\n"
		    "        example: -p /dir1/dir2/ -a file_inside_dir2    # a /full/path or regular ./relative/paths aren't allowed, and "
		    "would "
		    "get ignored\n"
		    "        -> if this option is used, only files that are in the allowed list would be synced\n"
		    "\n"
		    "    [-ap, --allow-pattern] <pattern>\n"
		    "        allow files/dirs with a certain pattern to be synced. this option can be used many times to *allow* multiple "
		    "patterns\n"
		    "        -> if this option is used, only files that are in the allowed list would be synced\n"
		    "\n"
		    "    [-F, --fsync]\n"
		    ...
		    "\n"
		    "    [-l, --link]\n"
		    "        hardlink regular files instead of copying them (works with files on the same machine (local to local) or "
		    "(remote to remote with the same user@ip))\n"
		    "        if source and destination files are on different machines, then it is normally copied\n"
		    "        -> if the destination file already exists, then it is removed then hardlinked from source\n"
		    "\n"
		    "    [-P, --progress]\n"
		    ...
		    "\n"
		    "    [-MFS, --max-file-size] <sizeUnit>\n"
		    "        don't sync a regular file if it is bigger than sizeUnit\n"
		    "        example: -p dir -p dir2 -MFS 0.5Tib\n"
		    "        example: -p dir -p dir2 -MFS 100mib\n"
		    "\n"
		    "    [-mfs, --min-file-size] <sizeUnit>\n"
		    "        don't sync a regular file if it is smaller than sizeUnit\n"
		    "        example: -p dir -p dir2 -mfs 0.5Tib\n"
		    "        example: -p dir -p dir2 -mfs 100mib\n"
		    "\n"
		    "    [-R, --resume]\n"
		    ...
		    "\n"
		    "    [-rmb, --remove-before]\n"
		    "        remove the destination *regular* file, if it exists, before updating it\n"
		    "\n"
		    "    [-rmp, --remove-partials]\n"
		    "        remove the files.ls.part, if an error occured which interrupted the file transfer\n"
		    "\n"
		    "    [-c, --config] <preset>\n"
		    ...
		    "\n"
		    "    [-post, --posthook] <cmd {attributes}>\n"
		    "        *inject* many posthooks/commands *after* every file that lunas tries to sync\n"
		    "        *retrieve* metadata from the source file\n"
		    "        example: -s dir1 -d dir2 --posthook \"/path/to/meow.sh {path} {attributes.size}\"\n"
		    "        -> read the man page for more details\n"
		    "\n"
		    "    [-V, --version]\n"
