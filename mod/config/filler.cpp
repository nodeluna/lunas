...
export namespace lunas
{
	namespace config
	{
		namespace filler
		{
			...
			std::expected<std::monostate, lunas::error> attributes_own(const std::string& data, lunas::config::options& options)
			{
				if (data == "on")
				{
					options.attributes_uid = true;
			...
			std::expected<std::monostate, lunas::error> attributes_all(const std::string& data, lunas::config::options& options)
			{
				if (data == "on")
				{
					options.attributes_atime = true;
			...
			std::expected<std::monostate, lunas::error> remove_before(const std::string& data, lunas::config::options& options)
			{
				if (data == "on")
				{
					options.remove_before = true;
				}
				else if (data == "off")
				{
					options.remove_before = false;
				}
				else
				{
					return std::unexpected(lunas::error(lunas::error_type::config_invalid_argument));
